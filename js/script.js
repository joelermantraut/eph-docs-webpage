...
document.addEventListener("DOMContentLoaded", function () {
  ...
  */
  ...
  fetchDocument(MANUAL_PATH, options).then((data) => {
    documentData = data;
  ...
  browser_el.addEventListener("keyup", function (event) {
    if (event.key === "Enter") {
      ...
      if (search_results.length > 0) {
        ...
        search_num.innerText =
          String(search_curr_index + 1) + "/" + String(search_results.length);
  ...
  search_next.addEventListener("click", function (event) {
    if (
      search_results != undefined &&
      search_curr_index < search_results.length - 1
    ) {
      search_curr_index++;
      id = search_results[search_curr_index].item.title;
      window.location.hash = id;
